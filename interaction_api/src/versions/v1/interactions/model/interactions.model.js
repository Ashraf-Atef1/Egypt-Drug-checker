const interactions = require("./interactions.mongo");
...
function getInteractionsNames(drugNames) {
    ...
    for(let i = 0; i < drugsArray.length - 1; i++)
        for(let j = i + 1; j < drugsArray.length; j++)
            InteractionNamesArray.push(new RegExp(drugsArray[i] + "\\+" + drugsArray[j], 'i'));
...
async function getInteractionsDB(drugNames) {
    ...
    const interactionsFromDB = await interactions.find({ name: { $in: InteractionsRegexList }});
...
async function addInteractionDB({name, interaction, type}) {
    ...
    const newInteraction = new interactions({ name, interaction, type });
